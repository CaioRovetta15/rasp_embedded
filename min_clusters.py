...
def generate_minimum_clusters(points, max_radius, min_points_per_cluster,plot_3d=False,plot_2d=False,x_lim=500,y_lim=1000,plot_size=(640,640)):
    ...
    # if plot_3d:
    #     #white opencv image
    #     img = np.ones((y_lim,x_lim,3), np.uint8)*255
    #     for i,point in enumerate(points):
    #         #convert to image coordinates

    #         cv2.circle(img, (int(point[0]), int(point[2])), 2, (0, 0, 255), -1)
    ...
    if plot_2d:
        ...
        for i, cluster in enumerate(clusters):
            ...
            cv2.circle(img, (cluster_image_center[0], cluster_image_center[1]), cluster_image_radius,
                       cluster_colors[i], 3)
